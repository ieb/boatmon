...
class OsMeter(Collector):
    ...
    def collect_cpu(self, source, hostname, nowSeconds, stat):
        ...
        for l in stat:
            if l.startswith('cpu'):
                ...
                cpuinfo.append(f'usage_user={diff[2]}')  # no   Looks like these may be shifte
                cpuinfo.append(f'usage_nice={diff[3]}')  # yes 0.015
                cpuinfo.append(f'usage_system={diff[4]}')  # yes 0.9
                cpuinfo.append(f'usage_idle={diff[5]}')   # o, some spikes
                cpuinfo.append(f'usage_iowait={diff[6]}') # 0.002 to 0.008
                cpuinfo.append(f'usage_irq={diff[7]}')  # 0.0005 to 0.002
                cpuinfo.append(f'usage_softirq={diff[8]}') # 0
                cpuinfo.append(f'usage_steal={diff[9]}') # 0
                cpuinfo.append(f'usage_guest={diff[10]}') # 0
                cpuinfo.append(f'usage_guest_nice={diff[11]}') #0 
    ...
    def collect_kernel(self, source, hostname, nowSeconds, stat):
        ...
        for l in stat:
            ...
            if s[0] == 'intr':
                # interrupts
                kernel.append(f'interrupts={s[1]}') # 1.6K/m
            elif s[0] == 'ctxt':
                kernel.append(f'context_switches={s[1]}') # 2k/m
            elif s[0] == 'btime':
                kernel.append(f'boot_time={s[1]}')  # ok
            elif s[0] == 'processes':
                kernel.append(f'processes_forked={s[1]}') # ok
            elif s[0] == 'procs_running':
                kernel.append(f'procs_running={s[1]}') # ok
            elif s[0] == 'procs_blocked':
                kernel.append(f'procs_blocked={s[1]}') # ok
        entropy = self.read_proc('/proc/sys/kernel/random/entropy_avail')
        kernel.append(f'entropy_avail={entropy[0].strip()}')
    ...
    def collect_system(self, source, hostname, nowSeconds, stat):
        ...
        for x in whoshere:
            u = x.split()
            unique_users[u[0]] = 1
        system.append(f'n_unique_users={len(unique_users)}')
        system.append(f'n_users={len(whoshere)}')

        return [ f'system,source={source},host={hostname} {",".join(system)} {nowSeconds}000000000' ]  # all ok
    ...
    def collect_process_stats(self, source, hostname, nowSeconds):
        ...
        fields_string_parts = [f"{key}={fields_data[key]}" for key in ordered_field_keys]
    ...
    def collect_mem(self, source, hostname, nowSeconds):
        ...
        mem.append(f'total={total}')
        mem.append(f'free={free}')
        mem.append(f'available={available}')
        mem.append(f'cached={cached}')
        #mem.append(f'sreclaimable={sreclaimable}')
        mem.append(f'used={used}')
        mem.append(f'used_percent={used_percent}')
        mem.append(f'available_percent={available_percent}')
        mem.append(f'active={mem_info.get("Active", "0")}')
        mem.append(f'buffered={mem_info.get("Buffers", "0")}')
        #mem.append(f'commit_limit={mem_info.get("CommitLimit", "0")}')
        #mem.append(f'committed_as={mem_info.get("Committed_AS", "0")}')
        #mem.append(f'dirty={mem_info.get("Dirty", "0")}')
        #mem.append(f'high_free={mem_info.get("HighFree", "0")}')
        #mem.append(f'high_total={mem_info.get("HighTotal", "0")}')
        #mem.append(f'huge_page_size={mem_info.get("Hugepagesize", "0")}')
        #mem.append(f'huge_pages_free={mem_info.get("HugePages_Free", "0")}')
        #mem.append(f'huge_pages_total={mem_info.get("HugePages_Total", "0")}')
        #mem.append(f'inactive={mem_info.get("Inactive", "0")}') 
        #mem.append(f'low_free={mem_info.get("LowFree", "0")}')  
        #mem.append(f'low_total={mem_info.get("LowTotal", "0")}') 
        #mem.append(f'mapped={mem_info.get("Mapped", "0")}') 
        #mem.append(f'page_tables={mem_info.get("PageTables", "0")}')
        #mem.append(f'shared={mem_info.get("Shmem", "0")}')
        #mem.append(f'slab={mem_info.get("Slab", "0")}')
        #mem.append(f'sunreclaim={mem_info.get("SReclaimable", "0")}')
        #mem.append(f'swap_cached={mem_info.get("SwapCached", "0")}')
        #mem.append(f'swap_free={mem_info.get("SwapFree", "0")}')
        #mem.append(f'swap_total={mem_info.get("SwapTotal", "0")}')
        #mem.append(f'vmalloc_chunk={mem_info.get("VmallocChunk", "0")}')
        #mem.append(f'vmalloc_total={mem_info.get("VmallocTotal", "0")}')
        #mem.append(f'vmalloc_used={mem_info.get("VmallocUsed", "0")}')
        #mem.append(f'write_back={mem_info.get("Writeback", "0")}')
        #mem.append(f'write_back_tmp={mem_info.get("WritebackTmp", "0")}')
        ...
        swap = []
        swap.append(f'free={int(mem_info.get("SwapFree",0))*10240}')
        swap.append(f'total={swap_total*10240}')
        swap.append(f'used={swap_used*10240}')
        swap.append(f'used_percent={swap_percent}')
        swap.append(f'in=0u')
    ...
    def collect_diskio(self, source, hostname, nowSeconds):
        ...
        for line in disk_info:
            ...
            diskinfo.append(f'io_time={diskstats[12]}')
            diskinfo.append(f'ops_in_progress={diskstats[11]}')
            diskinfo.append(f'merged_reads={diskstats[4]}')
            diskinfo.append(f'merged_writes={diskstats[8]}')
            diskinfo.append(f'read_bytes={int(diskstats[5])*512}')
            diskinfo.append(f'read_time={diskstats[6]}')
            diskinfo.append(f'reads={diskstats[3]}')
            diskinfo.append(f'weighted_io_time={diskstats[13]}')
            diskinfo.append(f'write_bytes={int(diskstats[9])*512}')
            diskinfo.append(f'write_time={diskstats[10]}')
            diskinfo.append(f'writes={diskstats[7]}')
    ...
    def collect_disk(self, source, hostname, nowSeconds):
        ...
        for k, v in fsa.items():
            if v[0].startswith('/dev/'):
                ...
                diskinfo.append(f'free={1024*int(fs[3])}')
                diskinfo.append(f'inodes_free={fs_inodes[3]}')
                diskinfo.append(f'inodes_total={int(inodes_total)}')
                diskinfo.append(f'inodes_used={fs_inodes[2]}')
                diskinfo.append(f'inodes_used_percent={inode_used_percent}')
                diskinfo.append(f'total={int(total)}')
                diskinfo.append(f'used={1024*int(fs[2])}')
...
root@nanopi:/data/grafana-exporter# 
        ...
        for line in net:
            ...
            if n[0].startswith('can') or n[0].startswith('eth') or n[0].startswith('wifi'):
                interface = []
                interface.append(f'bytes_recv={n[1]}')
                interface.append(f'bytes_sent={n[9]}')
                interface.append(f'drop_in={n[4]}')
                interface.append(f'drop_out={n[12]}')
                interface.append(f'err_in={n[3]}')
                interface.append(f'err_out={n[11]}')
                interface.append(f'packets_recv={n[2]}')
                interface.append(f'packets_sent={n[10]}')
        ...
        netstats.append(f'icmp_inaddrmaskreps={icmp_stats.get("InAddrMaskReps","0")}')
        netstats.append(f'icmp_inaddrmasks={icmp_stats.get("InAddrMasks","0")}')
        netstats.append(f'icmp_incsumerrors={icmp_stats.get("InCsumErrors","0")}')
        netstats.append(f'icmp_indestunreachs={icmp_stats.get("InDestUnreachs","0")}') 
        netstats.append(f'icmp_inechoreps={icmp_stats.get("InEchoReps","0")}')
        netstats.append(f'icmp_inechos={icmp_stats.get("InEchos","0")}')
        netstats.append(f'icmp_inerrors={icmp_stats.get("InErrors","0")}')
        netstats.append(f'icmp_inmsgs={icmp_stats.get("InMsgs","0")}')
        netstats.append(f'icmp_inparmprobs={icmp_stats.get("InParmProbs","0")}') 
        netstats.append(f'icmp_inredirects={icmp_stats.get("InRedirects","0")}') 
        netstats.append(f'icmp_insrcquenchs={icmp_stats.get("InSrcQuenchs","0")}') 
        netstats.append(f'icmp_intimeexcds={icmp_stats.get("InTimeExcds","0")}') 
        netstats.append(f'icmp_intimestampreps={icmp_stats.get("InTimestampReps","0")}') 
        netstats.append(f'icmp_intimestamps={icmp_stats.get("InTimestamps","0")}') 

                    
        netstats.append(f'icmp_outaddrmaskreps={icmp_stats.get("OutAddrMaskReps","0")}') 
        netstats.append(f'icmp_outaddrmasks={icmp_stats.get("OutAddrMasks","0")}')
        netstats.append(f'icmp_outdestunreachs={icmp_stats.get("OutDestUnreachs","0")}') 
        netstats.append(f'icmp_outechoreps={icmp_stats.get("OutEchoReps","0")}')
        netstats.append(f'icmp_outechos={icmp_stats.get("OutEchos","0")}') 
        netstats.append(f'icmp_outerrors={icmp_stats.get("OutErrors","0")}') 
        netstats.append(f'icmp_outmsgs={icmp_stats.get("OutMsgs","0")}') 
        netstats.append(f'icmp_outparmprobs={icmp_stats.get("OutParmProbs","0")}') 
        netstats.append(f'icmp_outredirects={icmp_stats.get("OutRedirects","0")}') 
        netstats.append(f'icmp_outsrcquenchs={icmp_stats.get("OutSrcQuenchs","0")}') 
        netstats.append(f'icmp_outtimeexcds={icmp_stats.get("OutTimeExcds","0")}') 
        netstats.append(f'icmp_outtimestampreps={icmp_stats.get("OutTimestampReps","0")}') 
        netstats.append(f'icmp_outtimestamps={icmp_stats.get("OutTimestamps","0")}') 

        icmpmsg_stats = snmp_map['IcmpMsg:']['data'] if 'IcmpMsg:' in  snmp_map else {} 
        netstats.append(f'icmpmsg_intype3={icmpmsg_stats.get("InType3","0")}')
        netstats.append(f'icmpmsg_outtype3={icmpmsg_stats.get("OutType3","0")}')



        ip_stats = snmp_map['Ip:']['data'] if 'Ip:' in  snmp_map else {} 
        netstats.append(f'ip_defaultttl={ip_stats.get("DefaultTTL","0")}')
        netstats.append(f'ip_forwarding={ip_stats.get("Forwarding","0")}') 
        netstats.append(f'ip_forwdatagrams={ip_stats.get("ForwDatagrams","0")}')
        netstats.append(f'ip_fragcreates={ip_stats.get("FragCreates","0")}')
        netstats.append(f'ip_fragfails={ip_stats.get("FragFails","0")}')
        netstats.append(f'ip_fragoks={ip_stats.get("FragOKs","0")}')
        netstats.append(f'ip_inaddrerrors={ip_stats.get("InAddrErrors","0")}')
        netstats.append(f'ip_indelivers={ip_stats.get("InDelivers","0")}')
        netstats.append(f'ip_indiscards={ip_stats.get("InDiscards","0")}')
        netstats.append(f'ip_inhdrerrors={ip_stats.get("InHdrErrors","0")}')
        netstats.append(f'ip_inreceives={ip_stats.get("InReceives","0")}')
        netstats.append(f'ip_inunknownprotos={ip_stats.get("InUnknownProtos","0")}')
        netstats.append(f'ip_outdiscards={ip_stats.get("OutDiscards","0")}')
        netstats.append(f'ip_outnoroutes={ip_stats.get("OutNoRoutes","0")}')
        netstats.append(f'ip_outrequests={ip_stats.get("OutRequests","0")}')
        netstats.append(f'ip_reasmfails={ip_stats.get("ReasmFails","0")}')
        netstats.append(f'ip_reasmoks={ip_stats.get("ReasmOKs","0")}')
        netstats.append(f'ip_reasmreqds={ip_stats.get("ReasmReqds","0")}')
        netstats.append(f'ip_reasmtimeout={ip_stats.get("ReasmTimeout","0")}')

        tcp_stats = snmp_map['Tcp:']['data'] if 'Tcp:' in  snmp_map else {}
        netstats.append(f'tcp_activeopens={tcp_stats.get("ActiveOpens","0")}')
        netstats.append(f'tcp_attemptfails={tcp_stats.get("AttemptFails","0")}')
        netstats.append(f'tcp_currestab={tcp_stats.get("CurrEstab","0")}')
        netstats.append(f'tcp_estabresets={tcp_stats.get("EstabResets","0")}')
        netstats.append(f'tcp_incsumerrors={tcp_stats.get("InCsumErrors","0")}')
        netstats.append(f'tcp_inerrs={tcp_stats.get("InErrs","0")}')
        netstats.append(f'tcp_insegs={tcp_stats.get("InSegs","0")}')
        netstats.append(f'tcp_maxconn={tcp_stats.get("MaxConn","0")}')
        netstats.append(f'tcp_outrsts={tcp_stats.get("OutRsts","0")}')
        netstats.append(f'tcp_outsegs={tcp_stats.get("OutSegs","0")}')
        netstats.append(f'tcp_passiveopens={tcp_stats.get("PassiveOpens","0")}')
        netstats.append(f'tcp_retranssegs={tcp_stats.get("RetransSegs","0")}')
        netstats.append(f'tcp_rtoalgorithm={tcp_stats.get("RtoAlgorithm","0")}')
        netstats.append(f'tcp_rtomax={tcp_stats.get("RtoMax","0")}')
        netstats.append(f'tcp_rtomin={tcp_stats.get("RtoMin","0")}')

        udp_stats = snmp_map['Udp:']['data'] if 'Udp:' in  snmp_map else {}
        netstats.append(f'udp_ignoredmulti={udp_stats.get("IgnoredMulti","0")}')
        netstats.append(f'udp_incsumerrors={udp_stats.get("InCsumErrors","0")}')
        netstats.append(f'udp_indatagrams={udp_stats.get("InDatagrams","0")}')
        netstats.append(f'udp_inerrors={udp_stats.get("InErrors","0")}')
        netstats.append(f'udp_noports={udp_stats.get("NoPorts","0")}')
        netstats.append(f'udp_outdatagrams={udp_stats.get("OutDatagrams","0")}')
        netstats.append(f'udp_rcvbuferrors={udp_stats.get("RcvbufErrors","0")}')
        netstats.append(f'udp_sndbuferrors={udp_stats.get("SndbufErrors","0")}')

        udplite_stats = snmp_map['UdpLite:']['data'] if 'UdpLite:' in  snmp_map else {}
        netstats.append(f'udplite_ignoredmulti={udplite_stats.get("IgnoredMulti","0")}')
        netstats.append(f'udplite_incsumerrors={udplite_stats.get("InCsumErrors","0")}')
        netstats.append(f'udplite_indatagrams={udplite_stats.get("InDatagrams","0")}')
        netstats.append(f'udplite_inerrors={udplite_stats.get("InErrors","0")}')
        netstats.append(f'udplite_noports={udplite_stats.get("NoPorts","0")}')
        netstats.append(f'udplite_outdatagrams={udplite_stats.get("OutDatagrams","0")}')
        netstats.append(f'udplite_rcvbuferrors={udplite_stats.get("RcvbufErrors","0")}')
        netstats.append(f'udplite_sndbuferrors={udplite_stats.get("SndbufErrors","0")}')
        # disable netstats
